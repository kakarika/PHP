...
<?php
if (!isLoggedIn()) {
    header('location: /sample%20project/notes/note-edit.php');
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($title) > 0 && strlen($body) > 0) {
        $update_query = sprintf(
            "UPDATE note SET `title`= '%s', `body` = '%s' WHERE id=%d AND user_id=%d",
...
if (isset($_GET['id'])) {
    ...
    $select_query = sprintf(
        "SELECT * FROM note where user_id = %d AND id = %d",
        mysqli_real_escape_string($conn, $user_id),
...
<?= $message ?? '' ?>
<?php if ($row) : ?>
    <form action="/sample%20project/notes/note-edit.php?id=<?= $note_id ?>" method="POST">
        <div class="mb-3">
